...
//显示等待
function showEasyDialog(msg){
	if(msg==null||msg==''){
		msg='数据装载中';
	}
	easyDialog.open({
		container : {
			content:'<div style="text-align:center;background-color: #fff;padding:8px 5px 0px 5px;"><img src="'+ctx+'/framework/js/easyui/skin/default/data-loading.gif" alt="'+msg+'" /><div>'+msg+'......</div></div>'
		}
	});
}
//关闭
function closeEasyDialog(time_ms){
	if(time_ms==null||time_ms==''){
		time_ms=600;
	}
	setTimeout("easyDialog.close()",time_ms);
}

function dialogSubmit(submitURL,redirectURL){

      showEasyDialog("与后台交互中请稍等...");
      $.ajax({
          type:'post',
          url:submitURL,
          cache:false,
