...
//	zTab()
	...
	if($('.z-close,.close'))
		$('.z-close,.close').click(function(){
			$('.blankDiv,.z-window').hide();
		})
	//表格隔行换色
	if($('.tableWrap table tr'))
	$('.tableWrap table tr').each(function(i){
		if(i%2 ==0){
			$(this).addClass("odd");
		}else{
			$(this).addClass("even");
    ...
    $('input:text,textarea').focus(function(){
		$(this).css({"border":"1px solid #489DD7"});
	}).blur(function(){
...
function openWin(url,width,height,closeCallback){
	var __cb = closeCallback || function(){};
	var __w = window.open(url, '_blank', 'height='+height+', width='+width+', top=15%, left=20%, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no,alwaysRaised=yes')
	var __watchClose = setInterval(function() {
	    if (__w.closed) {
	    	clearTimeout(__watchClose);
			__cb();
	    }
	 }, 500);
}
...
Validator.isPhoneNo = function(phoneNo){
	var patten =/^((\d{3,4})(-|\s))(\d{7,8})((-|\s)(\d{3,}))?$/;
	if(patten.test(phoneNo)) return true;
	else return false;
}

Validator.isMobelPhoneNo = function(phoneNo){
	var patten = /\d{11}/;
	if(patten.test(phoneNo)) return true;
	else return false;
}

Validator.isMail = function(mail){
	var patten = /^([\.a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(\.[a-zA-Z0-9_-])+/;
	if(patten.test(mail)) return true;
	else return false;
} 

Validator.isChineseChar = function(char){
	var patten = /[\u4e00-\u9fa5]+/;
	if(patten.test(char)) return true;
	else return false;
} 
